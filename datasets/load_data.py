...
PATH_TO_ORIGINAL_DATA = './recsys/'
PATH_TO_PROCESSED_DATA = '/recsys/processed_data'

data = pd.read_csv(PATH_TO_ORIGINAL_DATA + 'yoochoose-clicks.csv',
                   sep=',', header=None, usecols=[0,1,2], dtype={0:np.int32, 1:str, 2:np.int64})
data.columns = ['SessionId', 'TimeStr', 'ItemId']
# print(data)
data['Time'] = data.TimeStr.apply(lambda x:
                                  dt.datetime.strptime(x, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp()) #This is not UTC. It does not really matter.
del(data['TimeStr'])
